...
public interface ArtificialStep {
    default Throwable executeStep(String stepTitle, long expectedExecutionTime) {
        ...
        try{
            ...
            LOG.error("The following exception was throws : \n", e);
            Allure.LIFECYCLE.fire(new StepStartedEvent(name: "").withTitle(stepTitle + " (Exception was triggered)"));
        ...
        Allure.LIFECYCLE.fire(new StepStartedEvent(name: "").withTitle(stepTitle + " (" + execTime / 1000f + "s"));
        for ( MakeAttachmentEvent attachmentEvent : attachmentEvents ) {
            Allure.LIFECYCLE.fire(attachmentEvent);
        }
    ...
    long stepBody(List<MakeAttachmentEvent> attachments) throws Exception;
}
