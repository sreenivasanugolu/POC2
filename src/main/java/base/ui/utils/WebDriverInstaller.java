...
public class WebDriverInstaller {
    ...
    public void installDriver(String driverName, String driverPropertyName){
        ...
        try{
            ...
            File fileSource = File.createTempFile(prefix: "driverName", suffix: "");
            FileUtils.copyInputStreamToFile(getClass().getResourceAsStream(name: subFolder + driverName), fileSource);
            if(!FileUtils.contentEquals(fileSource, fileTarget)){
                FileUtils.copyFile(fileSource, fileTarget);
                fileTarget.setExecutable(executable: true, ownerOnly: true);
            }
        }catch (IOException e){
